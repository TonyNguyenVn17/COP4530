...
TEST_CASE("Project 3 Test Cases for GritVM (EXTRA CREDIT!)") {
  ...
  SECTION("GritVM produces proper output for fact.gvm (Factorial calculation)") {
    long n = (rand() % 10) + 1, n1 = (rand() % 10) + 1, n2 = (rand() % 10) + 1;

    std::function<long(long)> func = [&func](long n) -> long {
      if (n <= 1) return 1;
      return n * func(n - 1);
    };
    initialMemory = { n };
    checkMemory = { n, func(n), n + 1 };
    vm.load("fact.gvm", initialMemory);
    vm.run();
    REQUIRE(vm.getDataMem() == checkMemory);
